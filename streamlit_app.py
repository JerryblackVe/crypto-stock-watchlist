...
def check_alerts() -> None:
    ...
    for symbol, meta in watchlist.items():
        ...
        if price is None:
            continue
        # Determine whether price crosses the alert threshold (either above or below)
        crossed = price >= alert_level_float or price <= alert_level_float
        ...
        if crossed:
            ...
            if not last_sent_ts or (now_ts - last_sent_ts > 3600):
                ...
                body = (
                    ...
                    f"que ha cruzado el nivel de alerta {alert_level_float:.2f}."
                )
...
def main() -> None:
    ...
    if symbols:
        ...
        for sym in symbols:
            ...
            # Safely obtain the alert level even if the entry isn't a dict
    ...
    if symbols:
        ...
        if st.button("Guardar alertas"):
            for sym in symbols:
                # Ensure each entry is a dictionary before assignment
...
if __name__ == "__main__":
    main()def fetch_current_price(symbol: str) -> float:
    """Fetch the latest price for a given ticker using yfinance.
...
def main() -> None:
    ...
    if symbols:
        ...
        for sym in symbols:
            price = fetch_current_price(sym)
            alert_level = watchlist[sym].get("alert", "")
            rows.append(
                {
                    "Ticker": sym,
                    "Precio actual": f"{price:.2f}" if price is not None else "—",
    ...
    if symbols:
        ...
        alert_values = {}
        for sym in symbols:
            current_alert = watchlist[sym].get("alert")
            try:
                current_val = float(current_alert)
            except Exception:
                current_val = 0.0
    ...
    if symbols:
        ...
        if selected_symbol and selected_symbol != "—":
            ...
            if data.empty:
                ...
                # Add alert level line if configured
                alert_val = watchlist[selected_symbol].get("alert")
                try:
                    alert_float = float(alert_val)
                    fig.add_hline(
                        y=alert_float,
